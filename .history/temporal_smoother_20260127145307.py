...
class TemporalSmoothingPipeline:
    ...
    def process_video(self, cnn_confidences: List[float]) -> Dict:
        ...
        weighted_var = np.sum((confidences - smoothed_score) ** 2 * attn_weights)
        
        # Check stability (low variance = stable)
        is_stable = weighted_var < 0.1
        
        # Decision
        prediction = "Live" if smoothed_score > self.threshold else "Spoof"
        
        return {
            'prediction': prediction,
            'smoothed_confidence': float(smoothed_score),
